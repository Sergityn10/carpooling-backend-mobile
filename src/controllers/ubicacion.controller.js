...
exports.registrarUbicacion = async (req, res) => {
  try {
    ...
    if (!viajeId || !usuarioDNI || latitud === undefined || longitud === undefined) {
      return res.status(400).json({ 
        error: 'viajeId, usuarioDNI, latitud y longitud son requeridos' 
      });
    }
...
exports.registrarUbicacionesBatch = async (req, res) => {
  try {
    ...
    for (let i = 0; i < ubicaciones.length; i++) {
      ...
      if (!u.viajeId || !u.usuarioDNI || u.latitud === undefined || u.longitud === undefined) {
      ...
      // Buscar usuario
      const usuario = await prisma.usuario.findUnique({
        where: { dni: u.usuarioDNI }
      });

      if (!usuario) {
        errores.push({ index: i, error: 'Usuario no encontrado' });
        continue;
      }
...
exports.obtenerUltimaUbicacion = async (req, res) => {
  try {
    ...
    if (!ubicacion) {
      return res.status(404).json({ error: 'No hay ubicaciones registradas para este viaje' });
    }
...
exports.obtenerUbicacionesUsuario = async (req, res) => {
  try {
    const { viajeId, usuarioDNI } = req.params;
...
exports.calcularDistancia = async (req, res) => {
  try {
    ...
    if (ubicaciones.length < 2) {
      return res.json({
        distanciaKm: 0,
        mensaje: 'Se necesitan al menos 2 ubicaciones para calcular la distancia'
      });
    }
