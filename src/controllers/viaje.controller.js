  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        error:
          "Faltan campos requeridos: conductorId, matricula, y coordenadas de inicio/fin",
      });
    }
...
exports.unirseViaje = async (req, res) => {
  try {
    const { codigoQR, pasajeroId } = req.body;

    if (!codigoQR || !pasajeroId) {
      return res.status(400).json({
        error: "Código QR y ID del pasajero son requeridos",
      });
    }
